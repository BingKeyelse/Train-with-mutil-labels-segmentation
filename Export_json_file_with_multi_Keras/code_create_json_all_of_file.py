...
from Export_json_file_with_multi_Keras.code_keras_to_import_to_create_json_all_of_files import Unet_detect_position
import os
import numpy as np
import base64
import io
...
def create_labelme_json(image_path, contour_points, save_path,name_image,keys):
    labelme_data = {
        "version": "5.4.1",
        "flags": {},
        "shapes": [],
        ...
        "imageHeight": 738,  # Cần cập nhật kích thước hình ảnh thực tế
        "imageWidth": 1024    # Cần cập nhật kích thước hình ảnh thực tế
    }

    # Tạo dữ liệu shape từ các điểm contour
    for idx,(contour,key) in enumerate(zip(contour_points, keys)):
        shape = {
            "label": key,
